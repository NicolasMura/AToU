<?php
  ...
  /* ------------------------------------------- Récupération infos action culturelle ----------------------------------- */
  ...
  {
    header("Location:actionsCulGestion.php");
  }
  // Récupération des infos présentes en bdd :
  $requete='SELECT ID, titre FROM actions_culturelles WHERE ID = '.$id;
  $reponse = $bdd->query($requete);
  $donneesPage = $reponse->fetch();
  
  /* ------------------------------------------------- Upload photos pour action culturelle ------------------------------------------------- */
  ...
  if(isset($_POST['flag']) AND $_POST['flag']==1)
  {
            ...
            if($sizePhoto <= $maxSizePhoto)
            {
              if(($dimensionsPhoto[0] <= $maxWidthPhoto) AND ($dimensionsPhoto[1] <= $maxHeightPhoto))
              {
                // Renommage du nom (attribut "alt" de la balise <img>) et du filename de la photo
                  // Récupération de l'ID de la dernière photo uploadée pour l'action cuturelle en cours d'ajout / modif
                $requeteID='SELECT MAX(ID) FROM photos WHERE actions_culturellesID = ' . $id;
                ...
                {
                  $nomPhoto = "Photo de l'action culturelle " . $donneesPage['titre'];
                  $donneesPage['titre'] = nettoyerChaine($donneesPage['titre']); // Nettoyage du nom de l'action culturelle pour la photo
                  $filenamePhoto = "action_" . $donneesPage['titre'] . "_" . $IDphoto . ".jpg";
                  ...
                  if($transfert)
                  {
                    ...
                    $requete='INSERT INTO photos(nom, filename, actions_culturellesID) VALUES(?, ?, ?)';
                    $reponse = $bdd->prepare($requete);
                    $reponse->execute(array($nomPhoto, $filenamePhoto, $id)) or die(print_r($bdd->errorInfo()));
                    
                    // Conversion de la photo en version vignette (champ "actions_culturellesID" à mettre à jour !)
                    ...
                    // Conversion de la photo en version mobile (champ "actions_culturellesID" à mettre à jour !)
                    //imagethumb('../photosOriginales/' . $filenamePhoto, '../photosMobiles/' . $filenamePhoto, 640);
                    
                    // Conversion de la photo en version version vignette mini (champ "actions_culturellesID" à mettre à jour !)
                    imagethumb('../photosOriginales/' . $filenamePhoto, '../photosVignettesMini/' . $filenamePhoto, 200);

                    // Conversion de la photo en version version HD pour Fancybox (champ "actions_culturellesID" à mettre à jour !)
    ...
    else $erreur = "Vous devez ajouter une action culturelle avant d'uploader une photo.";
  }
  ...
  /* ------------------------------------------------- Suppression photos de l'action culturelle -------------------------------------------- */
  ...
  /* ------------------------------------------------- Recensement photos uploadées pour action culturelle ---------------------------------- */
  
  // Récupération des éventuelles photos présentes en bdd :
  $requete='SELECT ID, filename FROM photos WHERE actions_culturellesID = '.$id.' ORDER BY ID';
  $reponse = $bdd->query($requete);
  $nombrePhotos = $reponse->rowCount();
  
  if($nombrePhotos > 0)
  {
    $requete="UPDATE actions_culturelles SET active = ? WHERE ID = ".$id;
    $req = $bdd->prepare($requete);
    $req->execute(array(1)) or die(print_r($req->errorInfo()));
    $req->closeCursor();

    $requeteEn="UPDATE actions_culturelles_en SET active = ? WHERE ID = ".$id;
  ...
  else
  {
    ...
    $pasDePhoto = "</p>Vous n'avez pas encore de photos pour cette action culturelle.</p>";
  }
?>
<!DOCTYPE HTML>
<html>
<head>
  <meta charset="UTF-8">
  <title>Ajouter des photos à l'action culturelle</title>
...
<body>
  ...
  <div id="container">
    ...
    <?php
       ...
       get_fil_ariane(array('actionsCulGestion.php' => 'Actions culturelles', 'final' => 'Gestion des photos de '.$donneesPage["titre"]));
    ?>
    </div> <!-- fin du fil d'Arianne -->
      
    <div class="halfPage filaire">
      <h1>Ajouter une photo à l'action culturelle</h1>
      
      <?php //echo "memory_limit : ".ini_get('memory_limit');?>
      <p><b><?php echo $avertissement;?></b></p>
      <p>Pour chaque action culturelle, la 1ère photo uploadée doit être au format paysage.</p>
      <p>Pour un affichage optimal sur le site, le format des photos préconisé est de 2/3. </p>
      <form id="form1" action="actionsCulPhotosGestion.php?ID=<?php echo $donneesPage["ID"];?>" method="post" enctype="multipart/form-data" >
        <p>
        ...
        <input type="submit" name="button" id="button" value="Ajouter à l'action culturelle" class="button-grand">
    ...
    <form class="formCheckbox" id="form2" action="actionsCulPhotosGestion.php?ID=<?php echo $donneesPage["ID"];?>" method="post" enctype="multipart/form-data" >
      <?php
        // S'il existe des photos de l'action culturelle en base de données, on les affiche
        if(isset($pathPhoto))
        {
          echo "<p>Vous avez " . $nombrePhotos . " photo(s) enregistrée(s) pour cette action culturelle :</p>";
          $k=0;
