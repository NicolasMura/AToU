<?php
          ...
          if($sizePhoto <= $maxSizePhoto)
          {
            if(($dimensionsPhoto[0] <= $maxWidthPhoto) AND ($dimensionsPhoto[1] <= $maxHeightPhoto))
            {
              ...
              if($dimensionsPhoto[0] > $dimensionsPhoto[1])
              {
                ...
                $filenamePhoto = "mobile_creation_" . $donneesPage['titre'] . ".jpg";
                ...
                if($transfert)
                {
                  ...
                  $requete='UPDATE creations SET filenamePhotoMobile = ? WHERE ID = '.$id;
  ...
  if(isset($_POST["button2"]))
  {
    ...
    if(count($_POST)>1)
    {
      ...
      $requete='SELECT filenamePhotoMobile FROM creations WHERE ID = ' . $id;
      $reponse = $bdd->query($requete);
      ...
      if($donnees) // Vérif supplémentaire pour correction bug de réactualisation de page juste après la suppression en bdd
      {
        ...
        $fichierAsupprimer4 = "../photosMobiles/" . $donnees["filenamePhotoMobile"];
        $fichierAsupprimer5 = "../photosVignettesMini/" . $donnees["filenamePhotoMobile"];
...
<body>
  ...
  <div id="container">
    ...
    <div class="halfPage filaire">
      ...
      <form id="form1" action="creationsPhotosMobilesGestion.php?ID=<?php echo $donneesPage["ID"];?>" method="post" enctype="multipart/form-data" >
        <p>
    ...
    <form class="formCheckbox" id="form2" action="creationsPhotosMobilesGestion.php?ID=<?php echo $donneesPage["ID"];?>" method="post" enctype="multipart/form-data" >
        <?php
            ...
            if($donnees["filenamePhotoMobile"] != "")
            {
                echo "<p>Vous avez 1 photo enregistrée pour cette création :</p>";
                    ...
                    <div>
                        ...
                        <input type="checkbox" name="suppPhoto" 
                               id="suppPhoto" value="0" >
                    </div>
                    <?php
