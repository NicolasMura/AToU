<?php
  ...
  $requete='SELECT ID, titre FROM actualites WHERE ID = '.$id;
...
echo "</pre>";*/
            ...
            if(($dimensionsPhoto[0] <= $maxWidthPhoto) AND ($dimensionsPhoto[1] <= $maxHeightPhoto))
            {
              ...
              if($dimensionsPhoto[0] > $dimensionsPhoto[1])
              {
                ...
                if($transfert)
                {
                  ...
                  $requete='UPDATE actualites SET filenamePhotoMobile = ? WHERE ID = '.$id;
  ...
  if(isset($_POST["button2"]))
  {
    ...
    if(count($_POST)>1)
    {
      ...
      $requete='SELECT filenamePhotoMobile FROM actualites WHERE ID = ' . $id;
      $reponse = $bdd->query($requete);
...
<head>
  <meta charset="UTF-8">
  <title>Ajouter la photo de l'actualité mobile</title>
...
<body>
  ...
  <div id="container">
    ...
    <?php
       ...
       get_fil_ariane(array('actualitesGestion.php' => 'Gestion des actualités', 'final' => 'Gestion de la photo de l\'actualité mobile pour '.$donneesPage["titre"]));
    ?>
    ...
    <div class="halfPage filaire">
      ...
      <form id="form1" action="actualitesPhotosMobilesGestion.php?ID=<?php echo $donneesPage["ID"];?>" method="post" enctype="multipart/form-data" >
        <p>
      ...
      <form class="formCheckbox" id="form2" action="actualitesPhotosMobilesGestion.php?ID=<?php echo $donneesPage["ID"];?>" method="post" enctype="multipart/form-data" >
        <?php
          // Si la photo existe, en base de données, on l'affiche
          if($donnees["filenamePhotoMobile"] != "")
          {
            echo "<p>Vous avez 1 photo enregistrée pour cette création :</p>";         
          ?>
          <div>
